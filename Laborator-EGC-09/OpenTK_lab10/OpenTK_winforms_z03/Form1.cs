...
namespace OpenTK_winforms_z02 {
    public partial class Form1 : Form {
        ...
        private int[,] arrVertex = new int[100, 3];         //Stocam matricea de vertexuri; 3 coloane vor reține informația pentru X, Y, Z. Nr. de linii definește nr. de vertexuri.
        private int nVertex;
        
        private int[] arrPyramid = new int[100];       //Stocam matricea de vertexuri; 3 coloane vor reține informația pentru X, Y, Z. Nr. de linii definește nr. de vertexuri.
        ...
        private string pyramidList = "pyramidList.txt";
        ...
        private void loadPyramid() {
            ...
            //Testăm dacă fișierul există
            try {
                StreamReader fileReader = new StreamReader((pyramidList));
                nPyramid = Convert.ToInt32(fileReader.ReadLine().Trim());
                Console.WriteLine("Vertexuri citite: " + nPyramid.ToString());
                ...
                statusFiles = false;
                Console.WriteLine("Fisierul cu informații vertex <" + pyramidList + "> nu exista!");
                MessageBox.Show("Fisierul cu informații vertex <" + pyramidList + "> nu exista!");
            }
        }
        ...
        private void DeseneazaPyramidQ_Tex2() {
            if (brick == true) {
                GL.BindTexture(TextureTarget.Texture2D, textures[0]);
            } else {
                GL.BindTexture(TextureTarget.Texture2D, textures[1]);
            ...
            //GL.Normal3(0.0, 0.0, 1.0);
            GL.Begin(PrimitiveType.Quads);
            for (int i = 0; i < nPyramid; i = i + 4) {
                GL.TexCoord2(0.0, 0.5);
                GL.Vertex3(arrVertex[arrPyramid[i], 0], arrVertex[arrPyramid[i], 1], arrVertex[arrPyramid[i], 2]);
                GL.TexCoord2(0.5, 0.5);
                GL.Vertex3(arrVertex[arrPyramid[i + 1], 0], arrVertex[arrPyramid[i + 1], 1], arrVertex[arrPyramid[i + 1], 2]);
                GL.TexCoord2(0.5, 0.0);
                GL.Vertex3(arrVertex[arrPyramid[i + 2], 0], arrVertex[arrPyramid[i + 2], 1], arrVertex[arrPyramid[i + 2], 2]);
                GL.TexCoord2(0.0, 0.0);
                GL.Vertex3(arrVertex[arrPyramid[i + 3], 0], arrVertex[arrPyramid[i + 3], 1], arrVertex[arrPyramid[i + 3], 2]);
                GL.TexCoord2(0.0, 0.0);
                GL.Vertex3(arrVertex[arrPyramid[i + 4], 0], arrVertex[arrPyramid[i + 4], 1], arrVertex[arrPyramid[i + 4], 2]);
            }
            GL.End();
        }
